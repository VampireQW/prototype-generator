...
当前版本新增 **HTML PPT 生成**、**设计系统资产层** 和 **外部 AI 生成**：HTML PPT 可通过导入 PPTX 美化或逐页描述生成；设计系统、skills 和 craft 资产参考 OpenDesign 的组织方式，为不同产物类型注入更稳定的设计约束；外部 AI 生成可将结构化需求交给 Cursor、Codex 等 AI 编程工具继续生成和精修。
...
*   **外部 AI 生成**：支持「仅复制完整prompt（1-2页面）」和「AI IDE 项目包（多页面）」两种模式。
