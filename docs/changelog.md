...
## 2026-06-10
### 新增功能
- **AI IDE 项目包模式**：新增不调用第三方大模型的项目包生成入口，可将多页面需求、参考图、PPT 原始配图、设计系统、组件风格、主题色等结构化导出给 Cursor / Codex 等 AI 编程工具使用。
- **外部 AI 生成入口整合**：将「复制 Prompt」和「AI IDE 项目包」合并为统一入口，提供「仅复制完整prompt（1-2页面）」与「AI IDE 项目包（多页面）」两种模式。
- **结构化上下文文件**：自动生成 `AGENTS.md`、`TASK.md`、`DESIGN.md`、`pages/*.md`、`reference/manifest.json` 和 `README.md`，外部 AI 工具可直接读取并生成 `index.html`。
- **设计风格继承**：AI IDE 包会写入当前选择的设计系统原始 `DESIGN.md` 内容，并保留组件风格、背景模式、主色和强调色约束。
- **研发版页面导航可收起**：导出研发版 / 发布研发版的左侧页面列表支持向左收起，保留竖向入口，减少原型预览区域占用。

### 修复优化
- **导出研发版端类型自动检测**：优化发布研发版和导出研发版的自动视图检测，增强 Web 端 / 移动端识别逻辑，避免移动端页面被默认展示成网页端或反向误判。

### 文件修改
- `server.py`: 新增 `/create-ai-ide-package` 接口和 AI IDE 项目包文件生成逻辑。
- `export_project.py`: 同步研发版导出模板的端类型自动检测与左侧页面导航收起逻辑。
- `src/index.html`: 顶部整合「外部 AI 生成」入口和模式选择弹窗。
- `src/script.js`: 新增外部 AI 模式分发、AI IDE 项目包前端打包、图片元数据整理和启动指令复制逻辑。
- `src/viewer.html`, `src/viewer_standalone.html`: 增强研发版预览器自动检测与左侧页面导航收起体验。

## 2026-05-09
### 新增功能
- **HTML PPT 生成**：新增 `HTML PPT` 产物类型，支持「导入 PPT 美化」和「逐页描述生成」两种输入方式。
- **PPTX 解析**：新增 `/api/pptx/parse`，可从 `.pptx` 中提取页序、文字和原始配图；每页最多保留 5 张配图。
- **概览与播放模式**：HTML PPT 生成 Prompt 要求产物默认展示大缩略图概览，点击后进入全屏播放；播放中支持键盘、鼠标和触摸翻页，并通过 `Esc` 或右键退出。
- **设计系统资产层**：新增 `design-systems/`、`skills/`、`craft/`，参考 OpenDesign 的 skills 及资产组织方式，为不同产物类型注入设计系统、产物 skill 和通用 craft 规范。

### 优化
- **全局设置顺序**：调整为「产物类型 -> 设计系统 -> 背景模式 -> 组件风格 -> 主题色 -> 辅助色」，先确定生成目标，再选择设计语言。
- **设计系统选择体验**：设计系统改为中文展示，新增辅助说明问号和色卡预览，便于快速判断主配色。
- **HTML PPT 图片语义**：区分「PPT 原始配图」和「参考图」；PPT 配图绑定对应页面并可删除，参考图作为额外视觉参考。
- **提示词组合**：生成时统一注入设计系统、产物 skill 和 craft 上下文，优先级为用户输入 > 设计系统 > skill > craft > 表单颜色。

### 修复
- **生成失败可见性**：AI 响应异常、非 JSON 响应和 HTTP 错误会记录实际错误信息，异步失败会写入项目记录。
- **生成卡死处理**：超过 15 分钟仍处于生成中的项目会自动标记失败，并提示减少图片或切换模型重试。
- **多模态图片控制**：单次请求最多发送 16 张图片，并过滤 PPTX 中不支持的图片类型，减少模型侧失败。
- **项目列表去重**：保存项目列表时按项目 ID 去重，避免重复历史记录。

### 文件修改
- `server.py`: 新增设计系统/skills API、PPTX 解析、Prompt 资产注入、图片分组与生成失败处理。
- `src/index.html`: 调整全局设置顺序、新增设计系统说明与色卡容器、HTML PPT 输入区域结构。
- `src/script.js`: 新增产物类型联动、PPTX 上传解析、PPT 配图/参考图区分、HTML PPT Prompt 规则和图片数量控制。
- `design-systems/`, `skills/`, `craft/`: 新增 OpenDesign 风格资产与生成规范。
- `README.md`, `docs/`: 更新 HTML PPT、设计系统、API 与架构说明。

## 2026-05-06
### 优化
- **默认离线个人版**：主界面默认无需登录即可使用本地个人项目，支持创建、查看、预览和继续生成原型。
- **团队版按需登录**：只有切换到「团队项目」、团队管理或分享到团队时才提示登录，降低个人离线使用门槛。
- **本地项目自动导入账号**：登录 / 注册后自动将当前本地个人项目导入到登录用户的「我的项目」，并与离线个人版保持同步。
- **项目内友好弹窗**：统一替换浏览器默认 `alert` / `confirm` / `prompt`，删除、复制、团队登录确认、模型删除、退出团队、微调结果等操作改为项目内弹窗。

### 文件修改
- `server.py`: 放开离线个人版本地项目访问，并新增本地项目归属同步逻辑。
- `src/index.html`: 新增通用应用内弹窗容器和离线 / 团队版用户菜单状态。
- `src/script.js`: 新增项目内确认 / 输入 / 提示弹窗，团队版登录改为按需触发。
- `src/login.html`: 支持登录后回到团队版入口。
- `src/viewer.html`: 预览器微调成功 / 失败提示改为应用内弹窗。

## 2026-05-01
### 新增功能
- **团队协作功能**：新增注册 / 登录、会话 Cookie、团队创建、邀请码加入、成员查看、退出团队等能力。
- **项目归属与团队分享**：登录用户创建的项目会记录拥有者，项目拥有者可将项目分享给指定团队，团队成员可在「团队项目」中统一查看。
- **协作数据层**：新增 `db.py`，使用 SQLite 保存用户、会话、团队、团队成员、项目归属和项目分享关系。
- **登录入口**：新增 `src/login.html`，提供登录 / 注册页面，并与主界面的用户菜单、团队管理弹窗联动。
- **部署支持**：新增 `Dockerfile`、`fly.toml`、`start.sh`，支持 Docker / Fly.io 部署，并将数据目录挂载到持久化卷。

### 优化
- **项目访问控制**：团队分享项目支持通过 `/projects/` 访问控制校验，未登录访问会引导到登录页。
- **我的项目 / 团队项目切换**：主界面项目列表新增个人项目与团队项目视图，团队项目展示项目拥有者。
- **微调路径动态化**：预览器通过 `/api/server-info` 获取服务端项目目录，移除本地硬编码路径。
- **旧项目迁移**：新增 `migrate.py`，可将已有 `projects.json` 项目分配给指定用户。

### 文件修改
- `server.py`: 新增认证、团队、项目分享、项目过滤、访问控制和服务端路径接口。
- `db.py`: 新增协作数据库层。
- `src/index.html`: 新增用户菜单、项目标签切换、团队管理和分享弹窗。
- `src/script.js`: 新增登录态检测、团队管理、项目分享和团队项目加载逻辑。
- `src/login.html`: 新增登录 / 注册页面。
- `src/viewer.html`: 微调提示词中的项目文件路径改为动态生成。
- `Dockerfile`, `fly.toml`, `start.sh`: 新增容器化与 Fly.io 部署配置。
